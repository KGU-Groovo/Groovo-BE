...
class VideoControllerTest {
  ...
  void getVideo_returnsDetailWithoutKeypointPath() throws Exception {
    mockMvc
        .perform(get("/v1/videos/{videoId}", firstVideoId).header("Authorization", bearerToken()))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.success").value(true))
  ...
  void getVideo_returns404_whenMissing() throws Exception {
    mockMvc
        .perform(get("/v1/videos/{videoId}", 999999).header("Authorization", bearerToken()))
